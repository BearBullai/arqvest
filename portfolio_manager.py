...
class PortfolioManager:
    ...
    @staticmethod
    def get_portfolio_summary(securities=None):
    ...
    @staticmethod
    def calculate_portfolio_metrics(securities=None):
    ...
    @staticmethod
    def calculate_risk_metrics(securities=None):
    ...
    @staticmethod
    def record_snapshot(user_id=None, date=None):
        """Record a snapshot of the portfolio for the specified date"""
        if date is None:
            date = datetime.utcnow().date()
        
        if user_id:
            securities = Security.query.filter_by(user_id=user_id).all()
        else:
            securities = Security.query.all()
            
        total_value = sum(security.current_value for security in securities)
        
        # Check if a snapshot already exists for this date and user
        if user_id:
            existing_snapshot = PortfolioSnapshot.query.filter_by(user_id=user_id, date=date).first()
        else:
    ...
    @staticmethod
    def get_historical_performance(days=30, user_id=None):
    ...
    @staticmethod
    def generate_report(securities=None):
        """Generate a comprehensive portfolio report"""
        if securities is None:
            securities = Security.query.all()
            
        summary = PortfolioManager.get_portfolio_summary(securities)
        metrics = PortfolioManager.calculate_portfolio_metrics(securities)
        risk = PortfolioManager.calculate_risk_metrics(securities)
        
        # Get user_id from first security if available
        user_id = securities[0].user_id if securities else None
        performance = PortfolioManager.get_historical_performance(user_id=user_id)
